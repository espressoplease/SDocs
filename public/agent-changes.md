...
The Cloud edition now matches the Cloud-first CLI behavior. A normal `sdoc FILE.md` creates or updates the Cloud document before opening it, `+tag` arguments update Cloud tags, and the local Markdown remains the working copy. The standard edition is unchanged.

The standard edition installs this body:
...
The Cloud edition installs this body:
...
**Reason:** Runnable HTML guidance now limits components to cases where interaction communicates the result more clearly than static document forms.

The installed skill now gives runnable HTML a stricter decision boundary. It uses executable components when interaction carries meaning that prose, diagrams, charts, or computed cells cannot express as clearly, including manipulation, simulation, spatial navigation, and linked states.
...
The Cloud-aware edition replaces only the standard Cloud section above with
this body:
