...
Install the CLI, then teach your agent about `sdoc`:
...
Let the tool's purpose determine the visual structure. Start with a clear page, list, table, or form, or use a canvas or stage for spatial interaction. For repeated records, try divided rows before separate cards. Leave the inherited design tokens unchanged unless the brief calls for another visual direction, then add semantic colours, pills, badges, accent rails, gradients, or shadows when they communicate meaning. Run `sdoc apps` for the complete visual guidance and design-token reference.
...
An agent can attach explanations to source lines while opening a regular Markdown document:
...
A fenced block tagged with a language is syntax-highlighted in the browser. highlight.js is loaded lazily from CDN only when a code block is present, so a document with no code pays nothing.
...
This is not a unique identifier. It groups you with every other person who first visited that same week. Alongside the cohort, each visit also records a coarse device label (desktop / mobile / tablet), browser family (Chrome, Safari, etc.), a referrer category parsed from the standard HTTP headers, and the local hour and weekday from your device's clock at the time of the visit. If a homepage or short-link entry URL contains a source label such as `?src=email-launch`, that label is recorded in lowercase, with surrounding spaces removed. Sources are not inferred from referrers. A social short link can also opt into placement attribution with a public `pid`, such as `?src=x&pid=launch-reply-01`. A valid `pid` records the publishing placement and associated short-link id. It identifies a post, reply, or comment, not a visitor. Untagged short links do not receive placement attribution. Local `#md` document links are not source-attributed. None of these fields identifies you individually. Every 15 minutes, buffered visits are written to a local SQLite database. The dashboard at [smalldocs.org/analytics](https://smalldocs.org/analytics) links to separate homepage and short-link source views. Each view creates a weekly chart and table for every source label after its first recorded visit. The short-link view also shows opted-in placements. Source-attributed and placement-attributed visits remain part of the ordinary visit and short-link totals.
...
When the skill is itself updated (rare, only on releases that change the wording), sdoc prints a one-line summary plus a link to [the skill changelog](/agent-changes) so you can audit what changed. Because every agent resolves to the one canonical file, a refresh rewrites that single file and every symlinked agent picks it up.

If you previously ran an older `sdoc setup` that pasted an always-on SmallDocs section into your `CLAUDE.md` / `AGENTS.md` / `GEMINI.md`, running `sdoc setup` or `sdoc refresh` removes it (the skill replaces it, and leaving both would double-load the content). Your own notes in those files are preserved.
...
```bash
sdoc refresh     # rewrites the canonical skill (every symlink follows)
sdoc setup       # interactive: pick what to install
```
