...
This is not a unique identifier. It groups you with every other person who first visited that same week. Alongside the cohort, each visit also records a coarse device label (desktop / mobile / tablet), browser family (Chrome, Safari, etc.), a referrer category parsed from the standard HTTP headers, the local hour and weekday from your device's clock at the time of the visit, and a coarse social source label when the entry URL contains `?src=x`, `?src=yt`, or `?src=linkedin`, or the browser reports a recognized X, YouTube, or LinkedIn referrer. None of these fields identifies you individually. Every 15 minutes, buffered visits are written to a local SQLite database. The dashboard at [smalldocs.org/analytics](https://smalldocs.org/analytics) shows visit counts per cohort per week and a separate graph for visits attributed to those social sources. Source-attributed visits remain part of the ordinary visit and short-link totals.
